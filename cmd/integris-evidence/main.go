	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M3q openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p supervised CapEnter push first cut",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j); M3c–M3q CapEnter openat + release self-checks (incl. ambient FS-read) + supervised CapEnter push first cut",
