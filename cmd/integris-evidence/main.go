	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"engineering: Landlock/unveil/Seatbelt path allow-roots for Apply/Index/Journal/Audit via Runtime.AllowRoots; role-net deny (FreeBSD: jail ip4/ip6=disable before CapEnter, M3s)",
				...
				"engineering: Landlock/unveil/Seatbelt path allow-roots for Apply/Index via Runtime.AllowRoots; role-net deny (FreeBSD: jail ip4/ip6=disable before CapEnter, M3s)",
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M3s openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p supervised CapEnter push; M3r StrictLaunch CapEnter RestartOne; M3s FreeBSD jail ambient AF_INET deny",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r); M3c–M3s CapEnter openat + release self-checks (incl. ambient FS-read + FreeBSD jail AF_INET deny) + supervised CapEnter push + StrictLaunch CapEnter RestartOne",
