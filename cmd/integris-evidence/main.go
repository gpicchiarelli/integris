	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M5h openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p/M3y/M4b supervised CapEnter push (shared PSK + peer admit/deny); M3r/M3u/M3v/M3w/M3x/M3z/M4a StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j dual ExtraPeer + M2j apply + M2j parser-down); M3s FreeBSD ambient AF_INET residual; M3t FreeBSD sealed MAC key FD; M4c Darwin/OpenBSD anon key FD residual; M4d non-FreeBSD release ambient ROLE-NET deny; M4e–M4n Darwin Seatbelt StrictLaunch campaign; M4o–M4x Linux Landlock+seccomp StrictLaunch campaign; M4y–M5h OpenBSD pledge+unveil StrictLaunch campaign complete (push + RestartOne apply/parser-down/auth-primary/auth+audit ExtraPeer + peer-key Once + peer deny/admit + peer apply/parser-down RestartOne) + OpenBSD CI",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r/M3u/M3v/M3w/M3x/M3z/M4a/M4f/M4g/M4h/M4i/M4j/M4m/M4n/M4p/M4q/M4r/M4s/M4t/M4w/M4x/M4z/M5a/M5b/M5c/M5d/M5e/M5f/M5g/M5h); M3c–M5h CapEnter/Seatbelt/Landlock/pledge openat + release self-checks + supervised StrictLaunch push/RestartOne/peer deny-admit; FreeBSD sealed key FD (M3t); Darwin/OpenBSD anon key residual (M4c); ambient AF_INET residual (M3s); non-FreeBSD release ROLE-NET deny (M4d); Darwin Seatbelt campaign M4e–M4n; Linux Landlock StrictLaunch campaign M4o–M4x; OpenBSD pledge StrictLaunch campaign complete M4y–M5h (push+RestartOne apply/parser-down/auth-primary/auth+audit ExtraPeer+peer-key Once+peer deny/admit+peer apply/parser-down RestartOne)",
