	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M4u openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p/M3y/M4b supervised CapEnter push (shared PSK + peer admit/deny); M3r/M3u/M3v/M3w/M3x/M3z/M4a StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j dual ExtraPeer + M2j apply + M2j parser-down); M3s FreeBSD ambient AF_INET residual; M3t FreeBSD sealed MAC key FD; M4c Darwin/OpenBSD anon key FD residual; M4d non-FreeBSD release ambient ROLE-NET deny; M4e–M4n Darwin Seatbelt StrictLaunch campaign; M4o–M4u Linux Landlock+seccomp StrictLaunch push + RestartOne + peer-key Once push",
