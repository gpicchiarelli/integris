	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r/M3u/M3v/M3w/M3x); M3c–M3x CapEnter openat + release self-checks + supervised CapEnter push + StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j dual ExtraPeer); FreeBSD sealed key FD (M3t); ambient AF_INET residual (M3s)",
