	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r/M3u/M3v/M3w/M3x/M3z/M4a/M4f/M4g/M4h/M4i/M4j/M4m/M4n/M4p/M4q/M4r/M4s/M4t/M4w/M4x/M4z/M5a/M5b/M5c/M5d/M5e/M5f/M5g/M5h/M5i/M5j/M5k/M5l/M5m/M5n/M5o/M5p/M5q/M5r/M5s/M5t/M5u/M5v/M5w/M5x); M3c–M5x CapEnter/Seatbelt/Landlock/pledge openat + release self-checks + supervised StrictLaunch push/RestartOne/peer deny-admit; FreeBSD sealed key FD (M3t); Darwin/OpenBSD anon key residual (M4c); ambient AF_INET residual (M3s); non-FreeBSD release ROLE-NET deny (M4d); release ambient EXEC deny (M5o); release ambient FS-OPEN deny (M5p); NEG-FS-READ ENOENT honesty (M5q); NEG-FS-READ pre-apply existence (M5r); NEG-FS-WRITE unique+Require (M5s); archive path/write Available (M5t); Linux CapAmb clear (M5u); Linux no_new_privs Require (M5v); Linux seccomp TSYNC Require (M5w); Linux dumpable clear Require (M5x); Darwin Seatbelt campaign M4e–M4n; Linux Landlock StrictLaunch campaign M4o–M4x; OpenBSD pledge StrictLaunch campaign complete M4y–M5h + role-FS pledge tightening M5i + CI widen M5j + CI full daemon suite M5k + AllowRoots normalize child M5l + parent M5m + Linux inotify VNodeWatch M5n",
