	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M4k openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p/M3y/M4b supervised CapEnter push (shared PSK + peer admit/deny); M3r/M3u/M3v/M3w/M3x/M3z/M4a StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j dual ExtraPeer + M2j apply + M2j parser-down); M3s FreeBSD ambient AF_INET residual; M3t FreeBSD sealed MAC key FD; M4c Darwin/OpenBSD anon key FD residual; M4d non-FreeBSD release ambient ROLE-NET deny; M4e Darwin StrictLaunch Seatbelt push first cut; M4f Darwin StrictLaunch Seatbelt RestartOne apply; M4g Darwin StrictLaunch Seatbelt parser-down RestartOne; M4h Darwin StrictLaunch Seatbelt auth-primary RestartOne; M4i Darwin StrictLaunch Seatbelt auth ExtraPeer RestartOne; M4j Darwin StrictLaunch Seatbelt audit ExtraPeer RestartOne; M4k Darwin StrictLaunch Seatbelt peer-key Once push",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r/M3u/M3v/M3w/M3x/M3z/M4a/M4f/M4g/M4h/M4i/M4j); M3c–M4k CapEnter openat + release self-checks + supervised CapEnter push (PSK + peer admit/deny) + StrictLaunch CapEnter RestartOne; FreeBSD sealed key FD (M3t); Darwin/OpenBSD anon key residual (M4c); ambient AF_INET residual (M3s); non-FreeBSD release ROLE-NET deny (M4d); Darwin Seatbelt StrictLaunch push (M4e) + RestartOne apply (M4f) + parser-down RestartOne (M4g) + auth-primary RestartOne (M4h) + auth ExtraPeer RestartOne (M4i) + audit ExtraPeer RestartOne (M4j) + peer-key Once push (M4k)",
