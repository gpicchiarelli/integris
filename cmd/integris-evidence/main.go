	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M3s openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p supervised CapEnter push; M3r StrictLaunch CapEnter RestartOne; M3s FreeBSD ambient AF_INET residual (CapEnter does not deny; jail incompatible with allow-root rights)",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r); M3c–M3s CapEnter openat + release self-checks (incl. ambient FS-read) + supervised CapEnter push + StrictLaunch CapEnter RestartOne; FreeBSD ambient AF_INET residual documented",
