	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M3w openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p supervised CapEnter push; M3r/M3u/M3v/M3w StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j peer ExtraPeer); M3s FreeBSD ambient AF_INET residual; M3t FreeBSD sealed MAC key FD",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r/M3u/M3v/M3w); M3c–M3w CapEnter openat + release self-checks + supervised CapEnter push + StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j peer ExtraPeer); FreeBSD sealed key FD (M3t); ambient AF_INET residual (M3s)",
