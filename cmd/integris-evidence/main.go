	...
	campaigns := []struct {
		...
		},
		{
			...
			residual: []string{
				...
				"Journal/Audit path allow-roots landed; FreeBSD AllowRootFDs claimed (M3c); M3d–M3z openat/CapEnter chain; M3j drain; M3k–M3q release CapMode/rights/ambient FS-read; M3p/M3y supervised CapEnter push (shared PSK + peer keyring); M3r/M3u/M3v/M3w/M3x/M3z StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j dual ExtraPeer + M2j apply); M3s FreeBSD ambient AF_INET residual; M3t FreeBSD sealed MAC key FD",
				...
				"broader path allow-lists beyond archive caps still open",
				"Runtime.RestartChild + RestartPair (M2m) + RestartOne (M2n–M3j/M3r/M3u/M3v/M3w/M3x/M3z); M3c–M3z CapEnter openat + release self-checks + supervised CapEnter push (PSK + peer) + StrictLaunch CapEnter RestartOne (apply + parser-down + auth-primary + M2j dual ExtraPeer + M2j apply); FreeBSD sealed key FD (M3t); ambient AF_INET residual (M3s)",
