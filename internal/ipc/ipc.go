...
// (docs/security-architecture.md local IPC contract).
//
// Frames are length-prefixed, role-bound, sequenced, and size-capped before
// allocation. This package does not perform OS process isolation or crypto
// authentication; a MAC/AEAD binding is left to an accepted IP-C/IP-P.
package ipc
...
// Frame is one decoded IPC frame (payload not yet authenticated).
type Frame struct {
	Version      uint16
	Type         MessageType
...
// NewChannel constructs a channel with resource limits.
...
func (ch *ChannelState) Encode(typ MessageType, payload []byte) ([]byte, error) {
	...
	limits := resource.Limits{
		MaxBytes: uint64(ch.MaxFrame) + HeaderBytes, MaxCount: 1, MaxNesting: 1,
		MaxQueueDepth: ch.MaxQueue, MaxConcurrent: 1, MaxRetries: 1,
	}
	if err := limits.AdmitBytes(uint64(HeaderBytes + len(payload))); err != nil {
...
// Decode parses one frame and enforces role/nonce/sequence/queue policy.
func (ch *ChannelState) Decode(raw []byte) (Frame, error) {
	...
	if HeaderBytes+int(payLen) != len(raw) {
	...
	payload := append([]byte{}, raw[HeaderBytes:]...)
	ch.LastRecvSeq = seq
	ch.QueueDepth++
	if typ == TypeClose {
		ch.Closed = true
