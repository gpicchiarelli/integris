...
// TestM4hStrictLaunchSeatbeltRestartOneAuthPrimary is Darwin Seatbelt
...
// M3q/M4d fail-closed confine; second push succeeds (M3v Darwin parity).
func TestM4hStrictLaunchSeatbeltRestartOneAuthPrimary(t *testing.T) {
	bin := buildIntegrisd(t)
	psk := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(psk); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4h-seatbelt")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: psk,
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4h-seatbelt")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for auth-primary RestartOne ready under Seatbelt StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4h-seatbelt")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: psk,
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4h-seatbelt")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4h-seatbelt")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "audit.events")); err != nil {
...
// TestM4iStrictLaunchSeatbeltRestartOneAuthPeerExtraPeer is Darwin Seatbelt
...
// rebind under M3q/M4d fail-closed confine; second peer push admits again
// (M3w Darwin parity).
func TestM4iStrictLaunchSeatbeltRestartOneAuthPeerExtraPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4i-seatbelt")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4i-seatbelt")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for peer-auth RestartOne ready under Seatbelt StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4i-seatbelt")
	res := pushAfterRestart(t, remotesync.PushOptions{
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4i-seatbelt")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4i-seatbelt")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after ExtraPeer rebind under Seatbelt: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
...
// TestM4jStrictLaunchSeatbeltRestartOneAuditAuthExtraPeer is Darwin Seatbelt
...
// ExtraPeer→audit rebind under M3q/M4d fail-closed confine; second peer push
// admits again (M3x Darwin parity).
func TestM4jStrictLaunchSeatbeltRestartOneAuditAuthExtraPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(alice); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4j-seatbelt")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4j-seatbelt")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for audit ExtraPeer RestartOne ready under Seatbelt StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4j-seatbelt")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: alice, PeerID: "alice",
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4j-seatbelt")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4j-seatbelt")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after audit ExtraPeer rebind under Seatbelt: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
...
// TestM4lStrictLaunchSeatbeltPeerDenyAdmit is Darwin Seatbelt StrictLaunch
// coverage for M2i/M2j: peer keyring under Seatbelt; unknown peer and wrong-key
...
// (M4b Darwin parity).
func TestM4lStrictLaunchSeatbeltPeerDenyAdmit(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	...
	src, dst := t.TempDir(), t.TempDir()
	mustWrite(t, filepath.Join(src, "a.txt"), "hello-m4l")
	...
	if _, err := remotesync.Push(remotesync.PushOptions{
		...
		t.Fatal("expected unknown peer rejection under Seatbelt StrictLaunch")
	}
	assertFile(t, marker, "keep")
	if _, err := os.Lstat(filepath.Join(dst, "a.txt")); err == nil {
		t.Fatal("destination mutated after unknown peer under Seatbelt")
	}

	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr, Source: src, RootKey: alice, PeerID: "bob",
	}); err == nil {
		t.Fatal("expected wrong-key rejection under Seatbelt StrictLaunch")
	}
	assertFile(t, marker, "keep")
	...
	assertFile(t, filepath.Join(dst, "a.txt"), "hello-m4l")
	assertFile(t, marker, "keep")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
		t.Fatalf("journal under Seatbelt peer deny/admit: %v", err)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
		t.Fatalf("plan snapshot under Seatbelt peer deny/admit: %v", err)
	}
	...
	if !bytes.Contains(auditRaw, []byte("auth.peer.deny")) {
		t.Fatalf("audit missing auth.peer.deny under Seatbelt: %q", auditRaw)
	}
	if !bytes.Contains(auditRaw, []byte("auth.peer.admit")) {
		t.Fatalf("audit missing auth.peer.admit under Seatbelt: %q", auditRaw)
	}
}

// TestM4mStrictLaunchSeatbeltRestartOneApplyPeer is Darwin Seatbelt
...
// M3q/M4d fail-closed confine; second peer push admits again (M3z Darwin parity).
func TestM4mStrictLaunchSeatbeltRestartOneApplyPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(alice); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4m-seatbelt")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4m-seatbelt")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for peer apply RestartOne ready under Seatbelt StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4m-seatbelt")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: alice, PeerID: "alice",
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4m-seatbelt")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4m-seatbelt")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after peer apply RestartOne under Seatbelt: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
...
// TestM4nStrictLaunchSeatbeltRestartOneParserDownPeer is Darwin Seatbelt
// StrictLaunch coverage for M3u under M2j: peer keyring; kill parser after the
// first peer push; net+auth survive; parser→plan→index→apply→journal→audit
// respawn under M3q/M4d fail-closed confine; second peer push admits again
// (M4a Darwin parity).
func TestM4nStrictLaunchSeatbeltRestartOneParserDownPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(alice); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4n-seatbelt")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4n-seatbelt")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
		t.Fatalf("journal before peer parser-down: %v", err)
	}
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for peer parser-down RestartOne ready under Seatbelt StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4n-seatbelt")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: alice, PeerID: "alice",
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4n-seatbelt")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4n-seatbelt")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after peer parser-down under Seatbelt: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
		t.Fatalf("plan snapshot after peer parser-down: %v", err)
	}
}
