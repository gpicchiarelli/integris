...
// TestM4aStrictLaunchCapEnterRestartOneParserDownPeer is FreeBSD CapEnter
...
// respawn under M3m–M3q fail-closed confine; second peer push admits again.
func TestM4aStrictLaunchCapEnterRestartOneParserDownPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(alice); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4a-capenter")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4a-capenter")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for peer parser-down RestartOne ready under CapEnter StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4a-capenter")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: alice, PeerID: "alice",
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4a-capenter")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4a-capenter")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after peer parser-down under CapEnter: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
...
// TestM4bStrictLaunchCapEnterPeerDenyAdmit is FreeBSD CapEnter StrictLaunch
// coverage for M2i/M2j: peer keyring under CapEnter; unknown peer and wrong-key
...
func TestM4bStrictLaunchCapEnterPeerDenyAdmit(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	...
	src, dst := t.TempDir(), t.TempDir()
	mustWrite(t, filepath.Join(src, "a.txt"), "hello-m4b")
	...
	if _, err := remotesync.Push(remotesync.PushOptions{
		...
		t.Fatal("expected unknown peer rejection under CapEnter StrictLaunch")
	}
	assertFile(t, marker, "keep")
	if _, err := os.Lstat(filepath.Join(dst, "a.txt")); err == nil {
		t.Fatal("destination mutated after unknown peer under CapEnter")
	}

	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr, Source: src, RootKey: alice, PeerID: "bob",
	}); err == nil {
		t.Fatal("expected wrong-key rejection under CapEnter StrictLaunch")
	}
	assertFile(t, marker, "keep")
	...
	assertFile(t, filepath.Join(dst, "a.txt"), "hello-m4b")
	assertFile(t, marker, "keep")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
		t.Fatalf("journal under CapEnter peer deny/admit: %v", err)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
		t.Fatalf("plan snapshot under CapEnter peer deny/admit: %v", err)
	}
	...
	if !bytes.Contains(auditRaw, []byte("auth.peer.deny")) {
		t.Fatalf("audit missing auth.peer.deny under CapEnter: %q", auditRaw)
	}
	if !bytes.Contains(auditRaw, []byte("auth.peer.admit")) {
		t.Fatalf("audit missing auth.peer.admit under CapEnter: %q", auditRaw)
	}
}
