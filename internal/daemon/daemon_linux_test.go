...
// TestM4vStrictLaunchLandlockPeerDenyAdmit is Linux Landlock+seccomp
// StrictLaunch coverage for M2i/M2j: peer keyring under Landlock; unknown peer
// and wrong-key pushes are rejected without mutating the destination; a valid
// peer push admits with journal/audit/plan and auth.peer.deny +
// auth.peer.admit (M4b/M4l Linux parity).
func TestM4vStrictLaunchLandlockPeerDenyAdmit(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	...
	src, dst := t.TempDir(), t.TempDir()
	mustWrite(t, filepath.Join(src, "a.txt"), "hello-m4v")
	...
	if _, err := remotesync.Push(remotesync.PushOptions{
		...
		t.Fatal("expected unknown peer rejection under Landlock StrictLaunch")
	}
	assertFile(t, marker, "keep")
	if _, err := os.Lstat(filepath.Join(dst, "a.txt")); err == nil {
		t.Fatal("destination mutated after unknown peer under Landlock")
	}

	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr, Source: src, RootKey: alice, PeerID: "bob",
	}); err == nil {
		t.Fatal("expected wrong-key rejection under Landlock StrictLaunch")
	}
	assertFile(t, marker, "keep")
	...
	assertFile(t, filepath.Join(dst, "a.txt"), "hello-m4v")
	assertFile(t, marker, "keep")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
		t.Fatalf("journal under Landlock peer deny/admit: %v", err)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
		t.Fatalf("plan snapshot under Landlock peer deny/admit: %v", err)
	}
	...
	if !bytes.Contains(auditRaw, []byte("auth.peer.deny")) {
		t.Fatalf("audit missing auth.peer.deny under Landlock: %q", auditRaw)
	}
	if !bytes.Contains(auditRaw, []byte("auth.peer.admit")) {
		t.Fatalf("audit missing auth.peer.admit under Landlock: %q", auditRaw)
	}
}

// TestM4wStrictLaunchLandlockRestartOneApplyPeer is Linux Landlock+seccomp
...
// M3q/M4d fail-closed confine; second peer push admits again (M3z/M4m Linux
// parity).
func TestM4wStrictLaunchLandlockRestartOneApplyPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(alice); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4w-landlock")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4w-landlock")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for peer apply RestartOne ready under Landlock StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4w-landlock")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: alice, PeerID: "alice",
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4w-landlock")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4w-landlock")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after peer apply RestartOne under Landlock: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
...
// TestM4xStrictLaunchLandlockRestartOneParserDownPeer is Linux Landlock+seccomp
...
func TestM4xStrictLaunchLandlockRestartOneParserDownPeer(t *testing.T) {
	bin := buildIntegrisd(t)
	alice := make([]byte, remotesync.RootKeySize)
	if _, err := rand.Read(alice); err != nil {
		t.Fatal(err)
	...
	src1 := t.TempDir()
	mustWrite(t, filepath.Join(src1, "before.txt"), "before-m4x-landlock")
	if _, err := remotesync.Push(remotesync.PushOptions{
		Addr: addr1, Source: src1, RootKey: alice, PeerID: "alice",
	}); err != nil {
		t.Fatalf("first push: %v", err)
	}
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4x-landlock")
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "local.jrn")); err != nil {
	...
	case addr2 = <-ready:
	case <-time.After(45 * time.Second):
		t.Fatal("timeout waiting for peer parser-down RestartOne ready under Landlock StrictLaunch")
	}
	if addr2 != addr1 {
		t.Fatalf("listen addr changed: %q → %q", addr1, addr2)
	}
	...
	src2 := t.TempDir()
	mustWrite(t, filepath.Join(src2, "after.txt"), "after-m4x-landlock")
	res := pushAfterRestart(t, remotesync.PushOptions{
		Addr: addr2, Source: src2, RootKey: alice, PeerID: "alice",
	})
	if res.Outcome != "success" {
	...
	assertFile(t, filepath.Join(dst, "before.txt"), "before-m4x-landlock")
	assertFile(t, filepath.Join(dst, "after.txt"), "after-m4x-landlock")
	...
	if bytes.Count(auditRaw, []byte("auth.peer.admit")) < 2 {
		t.Fatalf("expected ≥2 auth.peer.admit after peer parser-down under Landlock: %q", auditRaw)
	}
	if _, err := os.Lstat(filepath.Join(dst, ".integris", "last-plan.json")); err != nil {
