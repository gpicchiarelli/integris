...
## M4k — Darwin StrictLaunch Seatbelt peer-key push (landed engineering)

- Darwin `TestM4kStrictLaunchSeatbeltPeerPushServe`: StrictLaunch Once with
...
## M4m — Darwin StrictLaunch Seatbelt peer apply RestartOne (landed engineering)

- Darwin `TestM4mStrictLaunchSeatbeltRestartOneApplyPeer`: peer keyring
  StrictLaunch under Seatbelt; kill apply after first peer push; net+auth+index
  + listen survive; apply+journal+audit respawn; ≥2 `auth.peer.admit` (M3z
  Darwin parity);
- not an IC-1 claim.

Exit: supervised Darwin Seatbelt peer apply RestartOne green;
`docs/daemon-m2a.md`.

## M4n — Darwin StrictLaunch Seatbelt peer parser-down RestartOne (landed engineering)

- Darwin `TestM4nStrictLaunchSeatbeltRestartOneParserDownPeer`: peer keyring
  StrictLaunch under Seatbelt; kill parser after first peer push; net+auth +
  listen survive; parser→plan→index→apply→journal→audit respawn; ≥2
  `auth.peer.admit` (M4a Darwin parity);
- not an IC-1 claim.
...
## M4p — Linux StrictLaunch Landlock+seccomp RestartOne apply (landed engineering)

- Linux `TestM4pStrictLaunchLandlockRestartOneApply`: StrictLaunch persistent
  serve under Landlock+seccomp; kill apply after first push; net+auth+index +
  listen survive; apply+journal+audit respawn under M3q/M4d fail-closed
  confine; second push succeeds (M3r/M4f Linux parity);
- FreeBSD CapEnter and Darwin Seatbelt RestartOne apply remain M3r/M4f; not an
  IC-1 claim.
...
## M4r — Linux StrictLaunch Landlock+seccomp RestartOne auth-primary (landed engineering)

- Linux `TestM4rStrictLaunchLandlockRestartOneAuthPrimary`: StrictLaunch
  persistent serve under Landlock+seccomp; kill auth after first push; net +
  full data plane + listen survive; auth respawns with primary peer rebind
  under M3q/M4d fail-closed confine; second push succeeds (M3v/M4h Linux
  parity);
- FreeBSD CapEnter and Darwin Seatbelt auth-primary RestartOne remain M3v/M4h;
  not an IC-1 claim.
...
## M4t — Linux StrictLaunch Landlock+seccomp RestartOne audit ExtraPeer (landed engineering)

- Linux `TestM4tStrictLaunchLandlockRestartOneAuditAuthExtraPeer`: StrictLaunch
  persistent serve with peer keyring under Landlock+seccomp; kill audit after
  first peer push; auth+net+parser+plan+index + listen survive;
  apply+journal+audit respawn with auth ExtraPeer→audit rebind; ≥2
  `auth.peer.admit` (M3x/M4j Linux parity);
- FreeBSD CapEnter and Darwin Seatbelt audit ExtraPeer RestartOne remain
  M3x/M4j; not an IC-1 claim.
...
## M4v — Linux StrictLaunch Landlock+seccomp peer deny/admit (landed engineering)

- Linux `TestM4vStrictLaunchLandlockPeerDenyAdmit`: StrictLaunch under
  Landlock+seccomp with peer keyring; unknown peer and wrong-key rejected
  without destination mutation; valid peer push admits with
  `auth.peer.deny` + `auth.peer.admit` (M4b/M4l Linux parity);
- FreeBSD CapEnter and Darwin Seatbelt peer deny/admit remain M4b/M4l; not an
  IC-1 claim.
...
## M5c — OpenBSD StrictLaunch pledge+unveil RestartOne auth ExtraPeer (landed engineering)

- OpenBSD `TestM5cStrictLaunchPledgeRestartOneAuthPeerExtraPeer`: peer
  keyring StrictLaunch; kill auth after first peer push; net + full data
  plane and listen survive; auth respawns with primary + audit ExtraPeer
  rebind under M3q/M4d fail-closed confine; ≥2 `auth.peer.admit`
  (M3w/M4i/M4s OpenBSD parity);
- OpenBSD CI `-run` includes M5c; not an IC-1 claim.

Exit: supervised OpenBSD pledge StrictLaunch RestartOne auth ExtraPeer green;
`docs/daemon-m2a.md`.

## M5d — OpenBSD StrictLaunch pledge+unveil RestartOne audit ExtraPeer (landed engineering)

- OpenBSD `TestM5dStrictLaunchPledgeRestartOneAuditAuthExtraPeer`: peer
  keyring StrictLaunch; kill audit after first peer push; auth+net+parser+
  plan+index and listen survive; apply+journal+audit respawn with auth
  ExtraPeer→audit rebind under M3q/M4d fail-closed confine; ≥2
  `auth.peer.admit` (M3x/M4j/M4t OpenBSD parity);
- OpenBSD CI `-run` includes M5d; not an IC-1 claim.

Exit: supervised OpenBSD pledge StrictLaunch RestartOne audit ExtraPeer green;
`docs/daemon-m2a.md`.

## M5e — OpenBSD StrictLaunch pledge+unveil peer-key Once push (landed engineering)

- OpenBSD `TestM5eStrictLaunchPledgePeerPushServe`: StrictLaunch Once under
  pledge+unveil with peer keyring completes a peer push with journal/audit/
  plan and ≥1 `auth.peer.admit` (M3y/M4k/M4u OpenBSD parity);
- OpenBSD CI `-run` includes M5e; not an IC-1 claim.
...
- `NegativeTraceCtl` + release `RequireTraceCtlDisabled` (Available or
  Skipped); `DISC-TRACE-CTL` Available on FreeBSD;
- DISABLE (not DISABLE_EXEC): CapEnter already denies execve; does not claim
  self cannot re-ENABLE (same class as re-`PR_SET_DUMPABLE`); FreeBSD parity
  with Linux dumpable / RLIMIT_CORE anti-trace surface; not an IC-1 claim.
