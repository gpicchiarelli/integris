...
FreeBSD non-net roles also attach a jail with `ip4=disable`/`ip6=disable`
before CapEnter so ambient AF_INET is denied (`RequireAmbientRoleNetDenied`,
M3s); CapEnter alone does not deny sockets. FreeBSD supervised StrictLaunch
push first cut under CapEnter is covered by M3p; StrictLaunch CapEnter
RestartOne first cut by M3r. Provisional
