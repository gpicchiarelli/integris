...
(`RequireAllowRootLimitFinding` M3n, `RequireConferredLimitFinding` M3o;
CapRightsGet want+absent verify M5y; FCNTL/IOCTL absent M6b) and
ambient path open is denied (`RequireAmbientFSReadDenied`, M3q) and ambient
path create outside allow-roots is denied (`RequireAmbientFSOpenDenied`, M5p).
...
`RLIMIT_CORE` soft=hard=0 (`RequireRlimitCoreZero`, M5z Linux / M6a
Unix-wide). Full empty
...
OpenBSD StrictLaunch pledge+unveil campaign M4y–M5h landed; M5i role-FS pledge tightening; M5j–M5k OpenBSD CI reaches FreeBSD parity (`go test ./...`).
Provisional
