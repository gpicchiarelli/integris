...
|---|---|
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (`CopyXattr` + `CopyBSDFlags` + `CopyACL` when cgo + `CopyTimes`); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
| Metadata / ACL | **adopted (Darwin cgo):** `platform.ACLRoundTrip` / `CopyACL` (`acl_*`); `CopyXattr` / `CopyBSDFlags` / `CopyTimes` (degraded path: `SyncFile` then `UtimesNano`) on CloneFile fallback; CapSparse/CapResourceFork/CapUnicode also empirical |
