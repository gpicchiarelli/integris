...
|---|---|
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin/Linux):** `platform.CloneFile` → Darwin `clonefile` / Linux `FICLONE` with exclusive-copy degraded fallback (sparse `SEEK_DATA`/`SEEK_HOLE` + `CopyXattr` + `CopyBSDFlags` + `CopyACL` when cgo + `CopyResourceFork` + `CopyTimes`); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (sparse `SEEK_DATA`/`SEEK_HOLE`; Linux `copy_file_range` before `io.Copy` + `CopyXattr` + `CopyBSDFlags` + `CopyACL` when cgo + `CopyResourceFork` + `CopyTimes`); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (sparse `SEEK_DATA`/`SEEK_HOLE` + `CopyXattr` + `CopyBSDFlags` + `CopyACL` when cgo + `CopyResourceFork` + `CopyTimes`); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch`; **adopted (Darwin/Linux/FreeBSD):** `platform.SendFile` → `sendfile(2)` to a connected socket (socketpair harness; OpenBSD ENOSYS / unavailable) |
| Metadata / ACL | **adopted (Darwin cgo):** `platform.ACLRoundTrip` / `CopyACL` (`acl_*`); `CopyXattr` / `CopyBSDFlags` / `CopyResourceFork` / `CopyTimes` (incl. Darwin birthtime via `Setattrlist` `ATTR_CMN_CRTIME`) + sparse extent copy on CloneFile fallback; CapUnicode also empirical |
| I/O completion | `kqueue`, `epoll`, accepted bounded `io_uring` adapters where IP-approved — **adopted (BSD/Darwin):** `platform.VNodeWatch` uses `kqueue`+`EVFILT_VNODE` (write/delete harness); Linux epoll / io_uring pending |
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (sparse `SEEK_DATA`/`SEEK_HOLE` + `CopyXattr` + `CopyBSDFlags` + `CopyACL` when cgo + `CopyResourceFork` + `CopyTimes`); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
| Metadata / ACL | **adopted (Darwin cgo):** `platform.ACLRoundTrip` / `CopyACL` (`acl_*`); **adopted (Linux):** POSIX ACL via `system.posix_acl_access` xattr; `CopyXattr` / `CopyBSDFlags` / `CopyResourceFork` / `CopyTimes` (incl. Darwin birthtime via `Setattrlist` `ATTR_CMN_CRTIME`) + sparse extent copy on CloneFile fallback; CapUnicode also empirical |
...
| Notification | `kqueue` VNODE / `NOTE_*`, `inotify`/`fanotify` only behind reviewed adapters — **adopted (Darwin/FreeBSD/OpenBSD):** `platform.VNodeWatch` (`NOTE_WRITE`/`NOTE_DELETE`); **adopted (Linux):** `platform.VNodeWatch` via `inotify` (`IN_MODIFY`/`IN_CLOSE_WRITE`, `IN_DELETE_SELF`; `DISC-INOTIFY`); fanotify / recursive watches pending |
