...
|---|---|
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (`CopyXattr` + `CopyACL` when cgo); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
| Metadata / ACL | **adopted (Darwin cgo):** `platform.ACLRoundTrip` / `CopyACL` (`acl_*`); `platform.CopyXattr` on degraded clone; CapACL/CapXattr probe + CloneFile consumers; CapBSDFlags/CapSparse/CapResourceFork/CapTimes/CapUnicode also empirical |
