...
| Metadata / ACL | **adopted (Darwin cgo):** `platform.ACLRoundTrip` / `CopyACL` (`acl_*`); `CopyXattr` / `CopyBSDFlags` / `CopyResourceFork` / `CopyTimes` + sparse extent copy on CloneFile fallback; CapUnicode also empirical |
