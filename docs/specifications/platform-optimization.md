...
|---|---|
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (`CopyACL` when cgo); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
| Metadata / ACL | **adopted (Darwin cgo):** `platform.ACLRoundTrip` / `CopyACL` (`acl_*`); CapACL probe + CloneFile degraded-copy consumer; CapXattr/CapBSDFlags/CapSparse/CapResourceFork/CapTimes/CapUnicode also empirical |
