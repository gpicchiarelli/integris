...
| I/O completion | `kqueue`, `epoll`, accepted bounded `io_uring` adapters where IP-approved — **adopted (BSD/Darwin):** `platform.VNodeWatch` uses `kqueue`+`EVFILT_VNODE` (write/delete harness); Linux epoll pending |
| Bulk transfer | `sendfile`, `copy_file_range`, `splice`, platform clone/reflink — **adopted (Darwin):** `platform.CloneFile` → `clonefile` with exclusive-copy degraded fallback (sparse `SEEK_DATA`/`SEEK_HOLE` + `CopyXattr` + `CopyBSDFlags` + `CopyACL` when cgo + `CopyResourceFork` + `CopyTimes`); consumer `recovery.FilePublisher.PublishFrom`; CapCOW discovery in `fsmodel.ProbeScratch` |
...
| Notification | `kqueue` VNODE / `NOTE_*`, `inotify`/`fanotify` only behind reviewed adapters — **adopted (Darwin/FreeBSD/OpenBSD):** `platform.VNodeWatch` (`NOTE_WRITE`/`NOTE_DELETE`); Linux inotify pending |
