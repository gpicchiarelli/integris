...
| Suite negotiation | wire + local allow-list | `TypeNegotiateOffer` carries suite IDs; `TypeNegotiateAccept` carries selected version+suite; `ConfirmAccept` fail-closed; `session.LocalSuites` ∩ offered |
