# Privilege-separated receive (M2a–M6c engineering increments)
...
- **M2k:** `-strict-launch` / `StrictLaunch` — full eight-role chain; children
  ...
  (M3m–M6c CapMode, Capsicum rights-limit, ambient FS-read deny, ambient
...
- **M5a:** OpenBSD StrictLaunch pledge+unveil RestartOne parser-down — kill
  parser after first push; net+auth + listen survive;
  parser→plan→index→apply→journal→audit respawn; second push succeeds
  (M3u/M4g/M4q OpenBSD parity)
- **M5b:** OpenBSD StrictLaunch pledge+unveil RestartOne auth-primary — kill
  auth after first push; net + full data plane + listen survive; auth
  respawns; second push succeeds (M3v/M4h/M4r OpenBSD parity)
- **M5c:** OpenBSD StrictLaunch pledge+unveil RestartOne auth ExtraPeer —
  peer keyring; kill auth after first peer push; net + full data plane +
  listen survive; auth respawns with ExtraPeer rebind; ≥2 `auth.peer.admit`
  (M3w/M4i/M4s OpenBSD parity)
- **M5d:** OpenBSD StrictLaunch pledge+unveil RestartOne audit ExtraPeer —
  peer keyring; kill audit after first peer push; auth+net+parser+plan+index
  + listen survive; apply+journal+audit respawn; ≥2 `auth.peer.admit`
  (M3x/M4j/M4t OpenBSD parity)
- **M5e:** OpenBSD StrictLaunch pledge+unveil peer-key Once push — peer
  keyring completes push with journal/audit/plan and ≥1 `auth.peer.admit`
  (M3y/M4k/M4u OpenBSD parity)
- **M5f:** OpenBSD StrictLaunch pledge+unveil peer deny/admit — unknown peer
  and wrong-key rejected without destination mutation; valid peer push admits
  with `auth.peer.deny` + `auth.peer.admit` (M4b/M4l/M4v OpenBSD parity)
- **M5g:** OpenBSD StrictLaunch pledge+unveil peer apply RestartOne — peer
  keyring; kill apply after first peer push; net+auth+index + listen survive;
  apply+journal+audit respawn; ≥2 `auth.peer.admit` (M3z/M4m/M4w OpenBSD parity)
- **M5h:** OpenBSD StrictLaunch pledge+unveil peer parser-down RestartOne —
...
- **M5j:** OpenBSD CI widen — `go test` all packages except full daemon
...
- **M5n:** Linux inotify VNodeWatch first cut — `platform.VNodeWatch` via
  inotify (write/delete; `DISC-INOTIFY`); closes Linux notification residual
- **M5o:** Release ambient EXEC deny fail-closed — `RequireAmbientExecDenied`
...
- **M5s:** NEG-FS-WRITE unique probe + readonly Require — nonce path;
  EEXIST → Unavailable; `RequireArchiveFSWriteDenied` for Index/Audit in
  release `Confine`
- **M5t:** archive allow-root path/write Available Require —
  `RequireArchiveFSPathAvailable` + `RequireArchiveFSWriteAvailable`
  (Apply/Journal) in release `Confine`
- **M5u:** Linux ambient capability clear — `PR_CAP_AMBIENT_CLEAR_ALL`
  + `RequireCapAmbientEmpty`; `DISC-CAP-EMPTY` Unavailable (CapBnd residual)
- **M5v:** Linux no_new_privs verify + Require — `PR_GET_NO_NEW_PRIVS` +
  `RequireNoNewPrivsSet`; CapBnd drop still needs `CAP_SETPCAP`
- **M5w:** Linux seccomp TSYNC + Require — `SECCOMP_SET_MODE_FILTER` +
  `SECCOMP_FILTER_FLAG_TSYNC` + `RequireSeccompFilter`; Landlock per-thread
  residual unchanged
- **M5x:** Linux dumpable clear + Require — `PR_SET_DUMPABLE(0)` +
  `RequireDumpableClear`; `NEG-PTRACE` remains observational (Yama)
- **M5y:** FreeBSD CapRightsGet verify — `LimitConferredFDs` /
  `LimitAllowRootFDs` fail closed unless want rights present and sentinel
  rights absent; `DISC-CAPSICUM`/`DISC-CAP-RIGHTS` Available; M3s AF_INET
  residual unchanged
- **M5z:** Linux RLIMIT_CORE=0 + Require — soft=hard=0 +
  `RequireRlimitCoreZero`; complements dumpable; `DISC-LANDLOCK` Available
  with per-thread residual explicit
- **M6a:** Unix-wide RLIMIT_CORE=0 — Darwin/OpenBSD/FreeBSD apply+verify
  before Seatbelt/unveil/CapEnter; release `RequireRlimitCoreZero` fail-closed
  on all confined Unix ports
...
Darwin/OpenBSD sealed key FD, broader product authz / PKI, or IC-1 evidence
campaigns.
