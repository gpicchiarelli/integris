# Privilege-separated receive (M2a–M5u engineering increments)
...
- **M2k:** `-strict-launch` / `StrictLaunch` — full eight-role chain; children
  ...
  (M3m–M5u CapMode, Capsicum rights-limit, ambient FS-read deny, ambient
...
- **M5u:** Linux ambient capability clear — `PR_CAP_AMBIENT_CLEAR_ALL`
  + `RequireCapAmbientEmpty`; dedicated-account residual for full empty sets
