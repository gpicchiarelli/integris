...
Vary memory, disk (`resource.WithSoftFSIZE` → EFBIG), file descriptors (`resource.WithSoftNOFILE`), CPU (`resource.WithSoftCPU` → SIGXCPU), queue depth, latency, loss, reordering,
