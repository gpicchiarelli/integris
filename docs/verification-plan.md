...
Vary memory (`resource.WithSoftAS` → ENOMEM on Linux/BSD; Darwin unenforceable), disk (`resource.WithSoftFSIZE` → EFBIG; Darwin `hdiutil` volume → ENOSPC), file descriptors (`resource.WithSoftNOFILE`), CPU (`resource.WithSoftCPU` → SIGXCPU), queue depth, latency, loss, reordering,
